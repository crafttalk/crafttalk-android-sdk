...
//                )
            ...
            is ItemListTag -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    result.setSpan(BulletSpan(10, ChatAttr.getInstance().colorTextOperatorMessage, 6), it.pointStart, it.pointEnd + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
                } else {
                    result.setSpan(BulletSpan(), it.pointStart, it.pointEnd + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
                }
            }
            is HostListTag -> {
                result.setSpan(LeadingMarginSpan.Standard(80), it.pointStart, it.pointEnd + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
            }
            is PhoneTag -> {
                result.setSpan(object : ClickableSpan() {
                    override fun onClick(widget: View) {
